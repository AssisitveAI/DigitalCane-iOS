...
- **탐색 방법**: 휴대폰이 향하는 방향에 있는 장소를 음성과 진동으로 알려드립니다.
- **지능형 자동 반경 조절 (Smart Radius)**: 앱이 스스로 장소의 밀도를 파악합니다. 번화가에서는 범위를 좁혀 소음을 줄이고, 한적한 곳에서는 범위를 넓혀 정보를 찾아줍니다.
- **정밀 POI 인식 (Hyper-Local Awareness)**: Overpass API의 기하학적 정보와 Google Places API의 명칭 정보를 결합한 **하이브리드 엔진**을 사용합니다. "건물 내부"와 같은 물리적 맥락뿐만 아니라 정확한 장소명까지 안내하며, 데이터가 없을 경우 상위 구역(캠퍼스, 공원) 정보를 제공하는 안전장치를 갖추고 있습니다.
