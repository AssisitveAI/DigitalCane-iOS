...
- **Vertical Scanning (카메라 모드)**: 스마트폰을 **세워서** 마치 사진을 찍듯이 전방을 비추며 탐색합니다. 이는 나침반 센서의 정확도를 높이고 사용자의 팔 피로도를 줄여주는 인체공학적 디자인입니다.
- **정밀 POI 인식 (Hyper-Local Awareness)**: Overpass API를 통해 **건물 형상(Polygon)** 뿐만 아니라 **상점(Node)** 데이터까지 분석합니다. 건물 내부 진입 여부는 물론, 편의점 바로 앞(1m) 도착까지 정밀하게 안내합니다.
