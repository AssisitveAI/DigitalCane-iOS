...
**DigitalCane**은 시각장애인의 독립적인 이동을 보조하기 위해 개발된 iOS 내비게이션 애플리케이션입니다. 사용자 주변의 편의 시설을 탐색하는 '디지털케인' 모드와, 대중교통 및 도보를 포함한 '대중교통경로안내' 기능을 제공합니다.
...
- **정밀 건물 인식 (Precision Awareness)**: Overpass API를 활용하여 건물의 실제 경계선(Polygon) 데이터를 분석, 사용자가 건물 **내부**에 있는지 정밀하게 판별하여 안내합니다.
...
### 2. 스마트 대중교통경로안내 (내비게이션)
단순한 길 안내를 넘어, 시각장애인에게 특화된 상세하고 친절한 대중교통경로안내를 제공합니다.
...
- **실시간 주소 안내**: `CLGeocoder`를 통해 현재 위치를 한글 주소로 변환하여 보여주며, 터치 시 음성으로 상세 주소(동, 번지 등)를 안내합니다.
- **유연한 SMS 전송**: 
  - 설정된 보호자뿐만 아니라, 방문 중인 지인 등 **원하는 수신자를 즉시 수정**하여 문자를 보낼 수 있습니다.
  - 전송 시 **현재 주소**와 **구글 지도 링크**가 자동으로 포함됩니다.
...
- **AI & Data**:
  ...
  - **Overpass API (OSM)**: 건물 형상 데이터 수집 (Ray Casting 판별용)
- **Accessibility & UX**: 
