...
- **정밀 실내/외 판별 및 POI 탐색**:
  - **OpenStreetMap Overpass API 연동**: 주변 **건물** 형상(Polygon)뿐만 아니라 **상점/시설(POI)**의 점(Node) 데이터까지 실시간으로 수집.
...
- **탐색 방법 (Vertical Scanning UX)**:
  - 기존 '리모컨 모드(Top Edge)'에서 **'업라이트 모드(Upright)'**로 탐색 자세 변경.
  - 사용자가 스마트폰을 세워 마치 사진을 찍듯이 정면을 응시하는 자연스러운 AR 자세를 유도하여 나침반 정확도 향상 및 팔의 피로도 감소.
- **가상 촉각 레이더 (Virtual Haptic Radar)**:
  - 스마트폰을 세워서 전방을 스캔(Scanning).
