...
struct HelpGuideView: View {
    ...
    let guides = [
        HelpItem(title: "디지털케인 (주변 탐색)", 
                 content: "휴대폰 상단이 정면을 향하도록 들고, 천천히 좌우로 부채질하듯 스캔하세요. '촉각 나침반' 기술이 적용되어, 전방 30미터 이내의 장소를 진동의 질감으로 입체적으로 전달합니다. 휠체어 이용자를 위해 입구가 편리한 곳은 별도로 안내해 드립니다. 방금 지나친 장소를 다시 확인하려면, 휴대폰을 다른 방향으로 돌렸다가 다시 그곳을 가리키면 됩니다.", 
                 iconName: "sensor.tag.radiowaves.forward.fill"),
        
        HelpItem(title: "대중교통 경로 안내 (내비게이션)", 
                 content: "하단의 마이크 버튼을 누르고 목적지를 말씀해 주세요. '버스로 가고 싶어'나 '지하철로 최단 시간'처럼 구체적으로 요청하시면 선호하는 수단으로 맞춤 경로를 찾아드립니다. 출발 전, 외출 준비를 돕기 위해 현재 날씨와 기온 정보도 브리핑해 드립니다.", 
                 iconName: "bus.fill"),
